"""
DQN Agent for Drone Inspection Path Planning
Implements a Deep Q-Network (DQN) agent for controlling drone inspection paths
...
class DQN_Agent:
    """DQN agent for drone control using image-based observations."""
    ...
    def _setup_model(self, tb_log_path: str):
        """Setup the DQN model with CNN policy."""
        return DQN(
            "CnnPolicy",
            self.env,
    ...
    def train(
        ...
        tb_log_name: str = "dqn_drone_airsim_run"
    ):
        """
        Train the DQN agent.
        ...
        eval_callback = EvalCallback(
            ...
            n_eval_episodes=5,
            best_model_save_path=".\\best_model\\dqn",
            log_path=".\\best_model_eval_logs\\dqn",
            eval_freq=100,
        )
    ...
    def save(self, model_name: str = "dqn_airsim_drone_policy"):
    ...
    def load(self, model_name: str):
        ...
        if os.path.exists(model_path):
            if self.model is not None:
                del self.model
            self.model = DQN.load(model_name, env=self.env)
